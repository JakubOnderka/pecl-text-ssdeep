...
PHP_ARG_ENABLE(libfuzzy, for ssdeep support,
[  --enable-libfuzzy       Use system libfuzzy library, enabled by default], yes, no)

if test "$PHP_SSDEEP" != "no"; then
  if test "$PHP_LIBFUZZY" != "no"; then
    ...
    PHP_ADD_INCLUDE(PHP_EXT_SRCDIR()/ssdeep)
    PHP_NEW_EXTENSION(ssdeep, ssdeep.c ssdeep/fuzzy.c, $ext_shared)
    PHP_SUBST(SSDEEP_SHARED_LIBADD)
  fi
else
